try {
window.LEDGER_MANAGER_VERSION = '20260813_1815';
...
// =========================================================================
// =========================================================================
// 🚀 パフォーマンステストデータ生成エンジン (安全・非同期ロード版)
// =========================================================================
window.generateMassiveDatasetOnMemory = function(onComplete) {
    try {
        ...
        // 日報データ (ブラウザの安定性を保つため 10,000件の超大容量に最適化)
        const reports = [];
        const workContents = [
        ...
        // DBのフック
        if (window.SiteDB) {
            window.SiteDB.getAll = function() { return window._massiveMemoryDb.sites; };
        }
        if (window.ReportDB) {
            window.ReportDB.getAll = function() { return window._massiveMemoryDb.reports; };
        }
        if (window.PurchaseDB) {
        ...
        console.error("Massive test generation error:", err);
        alert("テストデータ生成中にエラーが発生しました: " + err.message);
    }
};
        ...
        html += list.map(pur => {
            ...
            return `
                <tr style="border-bottom: 1px solid #333;">
                    ...
                    <td style="border: 1px solid #333; padding: 0.4rem; text-align: right;">${pur.listPrice ? '¥' + Math.round(pur.listPrice).toLocaleString() : '-'}</td>
                    ...
                    <td style="border: 1px solid #333; padding: 0.4rem; text-align: right;">${(pur.listPrice && pur.listPrice > 0 && pur.unitPrice > 0) ? ((pur.unitPrice / pur.listPrice) * 100).toFixed(1) + '%' : '-'}</td>
                </tr>
            `;
        }).join('');
...
function refreshPurchaseListTable(filter) {
    ...
    Object.keys(DEPT_INFO).forEach(key => {
        ...
        displayed.forEach(pur => {
            ...
            const listPriceText = pur.listPrice ? '¥' + Math.round(pur.listPrice).toLocaleString() : '-';
            const rateText = (pur.listPrice && pur.listPrice > 0 && pur.unitPrice > 0)
                ? ((pur.unitPrice / pur.listPrice) * 100).toFixed(1) + '%'
                : '-';

            tableRows += `
                <tr style="${bgStyle}">
                    <td style="padding: 0.75rem; text-align: center; border-right: 1px dashed var(--border-light); width: 45px;">
                    ...
                    <td style="padding: 0.75rem; text-align: right; font-family: 'Inter', sans-serif; color: var(--text-muted);">${listPriceText}</td>
                    <td style="padding: 0.75rem; text-align: right; font-family: 'Inter', sans-serif;">${pur.quantity}</td>
                    <td style="padding: 0.75rem; text-align: right; font-family: 'Inter', sans-serif;">¥${Math.round(pur.unitPrice).toLocaleString()}</td>
...
function renderLedgerList(container) {
    ...
    // 日報日付から締め月 (11日〜翌月10日を翌月度とする) を計算するヘルパー
...
function refreshLedgerTable(filter = {}) {
    ...
    const generateLedgerRow = (rep, showHolidayWork = false) => {
        ...
        const displaySiteName = rep.isOfficeWork ? `<span style="background:var(--color-warning); color:white; padding:0.1rem 0.35rem; border-radius:4px; font-size:0.7rem; margin-right:0.35rem; vertical-align:text-bottom;">事務仕事</span>${cleanDeptCode(siteName)}` : cleanDeptCode(siteName);
    ...
    submitBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (localStorage.getItem('use_local_server') === 'true') {
                ...
                // 【バグ修正】LocalStorageへの保存キーを db_v2.js の本物のキー 'report_ledger_sites' に修正
                localStorage.setItem('report_ledger_sites', JSON.stringify(allSites));
            }
...
function renderPartnerLedger(container) {
    ...
    // デフォルトで今月度を選択 (1日〜月末)
    const now = new Date();
    const currentClosingStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
...
function refreshPartnerLedgerTable(filter = {}) {
    ...
    for (let i = 0; i < allReports.length; i++) {
        ...
        for (let j = 0; j < partners.length; j++) {
            ...
            if (p) {
                const entry = Object.assign({}, r, { _targetPartner: p });
